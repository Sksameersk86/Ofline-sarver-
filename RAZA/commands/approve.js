        ...
        case "pending": {
          switch (body) {
                case `A`: {
                        ...
                        api.sendMessage(formatMessage(`» Successfully approved the box:\n${idBox}`), threadID, () => {
...
module.exports.run = async ({ event, api, args, Threads, handleReply, Users }) => {
  ...
  const cre = this.config.credits
        return api.sendMessage(formatMessage(`=====「 APPROVE 」=====\n\n${pb}${nmdl} l/list => see list of approved boxes\n\n${pb}${nmdl} p/pending => see the list of unapproved boxes\n\n${pb}${nmdl} d/del => with ID to remove from bot used list\n\n${pb}${nmdl} => Attach an ID to browse that box\n\n⇒ ${cre} ⇐`), threadID, messageID);
       }
      
    else if (args[0] == "del" || args[0] == "d") {
        idBox = (args[1]) ? args[1] : event.threadID;
      if (isNaN(parseInt(idBox))) return api.sendMessage(formatMessage("[ ERR ] Not a number"), threadID, messageID);
        if (!data.includes(idBox)) return api.sendMessage(formatMessage("[ ERR ] Box is not pre-approved!"), threadID, messageID);
      api.sendMessage(formatMessage(`[ OK ] Your group has been removed from the browsing list by the admin for the reason: ${lydo}`), idBox);
        api.sendMessage(formatMessage(`[ OK ] Box has been removed from the list of allowed bots`), threadID, () => {
        ...
        else api.sendMessage(formatMessage("[ OK ] ✨ApKa Group Approved Kar Diya Hai🙌.\n 🖤So Enjoy\n\n 🧸⚠️𝐌𝐀𝐃𝐄 𝐁𝐘: 𝐑𝐀𝐉 𝐗𝐖𝐃 💫\n 🖤𝚈𝚘𝚞 𝙲𝚊𝚗 𝙲𝚊𝚕𝚕 𝙷𝚒𝚖 𝐑𝐀𝐉 𝐗𝐖𝐃 ☠ 𝐑𝐀𝐉🖤\n😳𝐇𝐢𝐬 𝐅𝐚𝐜𝐞𝐛𝐨𝐨𝐤 𝐢𝐝🤓:- ☞ www.facebook.com/profile.php?id=61583409240142\n👋For Any Kind Of Help Contact On Telegram  Username 👉 @rajthakur5😇"), idBox, (error, info) => {
                api.changeNickname(` 〖 ${global.config.PREFIX} 〗 ➺ ${(!global.config.BOTNAME) ? "" : global.config.BOTNAME}`, idBox, global.data.botID);
      const axios = require('axios');
        const request = require('request');
        const fs = require("fs");
   let admID = "61583409240142";    
...
┗━━━   🧸 ━━━━┛\n=====================\n➪ BOT: ${global.config.BOTNAME}\n➪ Prefix: ${global.config.PREFIX}\n➪ Users: ${global.data.allUserID.length}\n➪ Groups: ${global.data.allThreadID.length}\n=====================\n[]---------------------------------------[]\nUse '${global.config.PREFIX}Help' T0o View The Commands That Available!(ღ˘⌣˘ღ)\n[]---------------------------------------[]\n⌨ Made by: ${firstname}\n`, mentions: [{
                ...
                if (error) return api.sendMessage(formatMessage("[ ERR ] Something went wrong, make sure the id you entered is valid and the bot is in the box!"), threadID, messageID);
                else {
                        data.push(idBox);
                        fs.writeFileSync(dataPath, JSON.stringify(data, null, 2));
                        api.sendMessage(formatMessage(`[ OK ] Successfully Approved The Box (◕‿◕):\n${idBox}`), threadID, () => {
          dataP.splice(dataP.indexOf(idBox), 1);
                fs.writeFileSync(dataPending, JSON.stringify(dataP, null, 2));
        }, messageID)
        }
